...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    if (!rateLimitResult.allowed) {
      return new Response(
        JSON.stringify({
          error: "Too many requests. Please wait before creating more notes.",
        }),
        {
          status: 429,
          headers: {
    ...
    let organizedContent = null;

    // If it's a transcription, organize the ideas and store both versions
    if (isTranscription) {
      console.log("Organizing transcription ideas...");
      organizedContent = await organizeIdeas(processedContent);
      // For transcriptions, use the organized content for title/tags generation
      processedContent = organizedContent;
    }
    ...
    // Save note to database with both content and organizedContent
    await database.execute({
      sql: `INSERT INTO notes (id, userId, title, content, organizedContent, tags, createdAt, updatedAt) 
            VALUES (?, ?, ?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`,
      args: [
        noteId,
        user.id,
        title,
        isTranscription ? sanitizeContent(content) : processedContent,
        organizedContent,
        JSON.stringify(tags),
      ],
    });
...
export const GET: APIRoute = async ({ request, url }) => {
  try {
    ...
    if (!rateLimitResult.allowed) {
      return new Response(
        JSON.stringify({
          error: "Too many requests. Please wait before fetching notes.",
        }),
        {
          status: 429,
          headers: {
    ...
    let sql = `SELECT id, title, content, organizedContent, tags, createdAt, updatedAt 
               FROM notes 
               WHERE userId = ?`;
    ...
    const notes = result.rows.map((row) => ({
      ...
      title: row.title,
      content: row.content,
      organizedContent: row.organizedContent,
